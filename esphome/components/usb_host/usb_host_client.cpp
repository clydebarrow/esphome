...
static void control_callback(usb_transfer_t *xfer) {
  auto client = reinterpret_cast<USBClient *>(xfer->context);
  client->control_transfer_callback(xfer);
}
void USBClient::control_transfer_callback(usb_transfer_t *xfer) {
...
void USBClient::control_transfer_(uint8_t type, bool data_in, uint8_t request, uint16_t value, uint16_t index,
                                  const transfer_cb_t callback, uint16_t length, uint8_t *data) {
  ...
  this->ctrl_transfer_data_->bEndpointAddress = data_in ? USB_DIR_IN : 0;
  this->ctrl_transfer_data_->num_bytes = sizeof(usb_setup_packet_t) + length;
  this->ctrl_transfer_data_->callback = control_callback;
